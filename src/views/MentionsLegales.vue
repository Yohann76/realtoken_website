  ...
  <div class="legal-page">
    <header class="legal-header">
      <div class="header-content">
        <p class="tag">{{ $t('mentions.tag') }}</p>
        <h1>{{ $t('mentions.pageTitle') }}</h1>
        <p class="lead">{{ $t('mentions.pageDescription') }}</p>
      </div>
    </header>
...
const blocks = computed(() => {
  const defs = [
    { type: 'h2', textKey: 'mentions.s1Title' },
    { type: 'p', textKey: 'mentions.s1Intro' },
    { type: 'ul', listKey: 'mentions.s1List' },
    { type: 'h2', textKey: 'mentions.s2Title' },
    { type: 'p', textKey: 'mentions.s2Content' },
    { type: 'h2', textKey: 'mentions.s3Title' },
    { type: 'ul', listKey: 'mentions.s3List' },
    { type: 'p', textKey: 'mentions.s3Disclaimer' }
  ]
  return defs.map((b) => {
